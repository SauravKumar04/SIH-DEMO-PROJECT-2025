...
export default function Appointments() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-emerald-50 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {appointments.length === 0 ? (
          <p className="text-gray-700">No appointments scheduled yet.</p>
        ) : (
          appointments.map((appt, index) => (
            <div
              key={index}
              className="bg-white shadow-lg rounded-xl p-6 animate-fadeIn border-l-4 border-secondary-color flex justify-between items-center"
            >
              <div>
                <h3 className="text-xl font-semibold text-primary-color">
                  {appt.doctor}
                </h3>
                <p className="text-gray-700">{appt.date}</p>
              </div>
              <button
                onClick={() => cancelAppointment(index)}
                className="bg-danger-color text-white px-4 py-2 rounded-lg shadow-md hover:bg-red-600 transition transform hover:-translate-y-1"
              >
                Cancel
              </button>
            </div>
