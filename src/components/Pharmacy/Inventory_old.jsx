...
export default function Inventory() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-purple-50 p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
            ...
            value={medicineName}
            onChange={(e) => setMedicineName(e.target.value)}
            className="border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-secondary-color transition"
          />
          <input
            type="number"
          ...
          <button
            onClick={addMedicine}
            className="bg-secondary-color text-white font-semibold rounded-lg shadow-md hover:bg-blue-600 transition transform hover:-translate-y-1"
          >
            Add
          </button>
        </div>
      ...
      {/* Inventory List */}
      <h2 className="text-2xl font-semibold text-primary-color mb-4">Current Stock</h2>
      {medicines.length === 0 ? (
        <p className="text-gray-700">No medicines in stock yet.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {medicines.map((med, index) => (
            <div
              key={index}
              className="bg-white shadow-lg rounded-xl p-6 border-l-4 border-accent-color flex justify-between items-center animate-fadeIn"
            >
              <div>
                <h3 className="text-xl font-semibold text-primary-color">{med.name}</h3>
                <p className="text-gray-700">Quantity: {med.quantity}</p>
              </div>
              <input
                type="number"
                value={med.quantity}
                onChange={(e) => updateQuantity(index, e.target.value)}
                className="border border-gray-300 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-secondary-color transition w-20 text-center"
              />
            </div>
          ))}
        </div>
